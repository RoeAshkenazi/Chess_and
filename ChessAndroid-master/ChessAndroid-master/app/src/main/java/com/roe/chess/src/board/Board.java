package com.roe.chess.src.board;
...
import com.roe.chess.src.player.Player;
...
public class Board implements Serializable {
	...
	public String getFEN() {
		...
		for (int i = 0; i < this.board.length; i++) {
			for (int j = 0; j < this.board[i].length; j++) {
				if(board[j][i] != null)
				{
					//Log.d("me:",board[j][i].getType()+" "+j+" "+i);
					s=s.concat(board[j][i].getType()+"");
				}
				else{
					s=s.concat("0");
				}
	...
	public void creatFromFEM(String fen){
		...
		this.board=b;
		this.turn=temp[b.length].charAt(0);
	}
